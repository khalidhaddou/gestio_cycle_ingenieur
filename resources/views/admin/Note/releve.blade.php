...
<body>
  ...
  <hr class="m-2">
    <h3 class=" text-center mt-4"><u>Relevé de notes</u></h3>
        ...
        <h6 class="mt-3">Nom et Prénom : {{$etudiant->nom}} {{$etudiant->prenom}} <span class="fw-bold float-end "> CNE: {{$etudiant->cne}}</span></h6>

    <h6>Filière : Cycle d'Ingénieurs "Génie Informatique" </h6>
    @foreach ($note as $semestre =>  $result )
    @php
            ...
            <table >
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    {{$noteFiinal=$item->note_final}}
                    {{$moyenne=$moyenne+$noteFiinal}}
                    {{$i++}}
                    <td class="td_table">{{$item->Nom}}</td>
